...
export default function TierSelector({ currentTier, isUnlocked, onTierChange, tierStats }: TierSelectorProps) {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex gap-2 lg:gap-3 p-2 lg:p-3 bg-theme bg-opacity-40 rounded-2xl backdrop-blur-md border border-theme">
        {tiers.map((tier) => (
          <motion.button
            key={tier.id}
            ...
            <motion.button
              ...
              className={`relative flex flex-col items-center gap-1 pt-4 px-3 pb-4 rounded-xl transition-all duration-300 min-w-[85px] flex-shrink-0 ${
                currentTier === tier.id
                  ? 'bg-willow-green text-willow-dark shadow-lg'
                  : isUnlocked[tier.id]
                  ? 'bg-theme bg-opacity-10 text-theme-primary'
                  : 'bg-theme-secondary bg-opacity-15 text-theme-secondary cursor-not-allowed'
              }`}
              whileTap={isUnlocked[tier.id] ? { scale: 0.95 } : {}}
            >
              ...
              {currentTier === tier.id && (
                <motion.div
                  className="absolute bottom-1 w-5 h-1 bg-willow-dark rounded-full"
                  layoutId="activeTierMobile"
                  initial={false}
                  transition={{ type: "spring", stiffness: 500, damping: 30 }}
                />
              )}
